...
export function HeatmapGrid({ event, responses }: HeatmapGridProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {responses.length > 0 && bestSlots.length > 0 && (
        <div className="bg-gradient-to-r from-cyan-900/20 to-green-900/20 border border-cyan-700 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xl font-bold text-foreground">Best Available Times</h3>
          </div>
      ...
      {/* Heatmap Grid */}
      <div>
        <h3 className="text-xl font-bold text-foreground mb-4">Availability Heatmap</h3>
                ...
                {uniqueTimes.map((timeLabel, timeIndex) => (
                  <tr key={timeIndex}>
                    ...
                    {Array.from(slotsByDate.entries()).map(([date, slots]) => {
                      ...
                      if (!data) return <td key={date} className="border border-border" />

                      const bgColor = getHeatmapColor(data.percentage, isDarkMode)
        ...
        <div className="mt-6 bg-card border border-border rounded-lg p-4">
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(0, isDarkMode) }} />
              <span className="text-sm text-foreground">0% (None)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(20, isDarkMode) }} />
              <span className="text-sm text-foreground">1-20%</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(40, isDarkMode) }} />
              <span className="text-sm text-foreground">21-40%</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(60, isDarkMode) }} />
              <span className="text-sm text-foreground">41-60%</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(80, isDarkMode) }} />
              <span className="text-sm text-foreground">61-80%</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded" style={{ backgroundColor: getHeatmapColor(100, isDarkMode) }} />

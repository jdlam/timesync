	...
	const steps = [
		{
			...
			description:
				"Set up your event with dates, times, and time slots. No account needed.",
		},
		{
			number: "2",
			title: "Share the Link",
			description:
				"Send the public link to participants. They can respond in seconds.",
		},
		{
			number: "3",
			title: "See Who's Available",
			description:
				"View the heatmap to instantly find the best time for everyone.",
		},
	];
	...
	return (
		<div className="min-h-screen bg-gradient-to-b from-background via-muted to-background">
			...
			<section className="relative py-20 px-6 text-center overflow-hidden">
				<div className="absolute inset-0 bg-gradient-to-r from-teal-500/10 via-emerald-500/10 to-teal-500/5"></div>
				<div className="relative max-w-5xl mx-auto">
					...
					<p className="text-xl md:text-2xl text-muted-foreground mb-8 max-w-3xl mx-auto">
						The fastest way to coordinate schedules. No signups, no hassle, just
						results.
					</p>
							...
							<div className="grid grid-cols-5 gap-2">
								...
								{["Mon", "Tue", "Wed", "Thu"].map((day, i) => (
									<div key={day} className="space-y-2">
										<div className="h-8 text-center text-muted-foreground font-semibold">
											{day}
										</div>
										<div className="h-10 rounded bg-teal-600/80 flex items-center justify-center text-white font-bold">
											{4 - i}
										</div>
			...
			<section className="py-20 px-6 max-w-7xl mx-auto">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{features.map((feature, index) => (
						<div
							key={index}
							className="bg-card backdrop-blur-sm border border-border rounded-xl p-6 hover:border-teal-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-teal-500/10"
						>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
						{steps.map((step, index) => (
							<div key={index} className="text-center">
								<div className="w-16 h-16 rounded-full bg-teal-600 text-white text-2xl font-bold flex items-center justify-center mx-auto mb-4">
									{step.number}
								</div>

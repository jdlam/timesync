  ...
  loader: async ({ params }) => {
    try {
      ...
      return {
        event: null,
        responses: [],
        error: error instanceof Error ? error.message : 'Event not found or invalid admin token'
      }
    }
  },
})
...
const getEventByAdminToken = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const [event] = await db
      ...
      .from(events)
      .where(
        and(eq(events.id, data.eventId), eq(events.adminToken, data.adminToken))
      )
      .limit(1)
...
function AdminDashboard() {
  const { event, responses: initialResponses, error: loaderError } = Route.useLoaderData()

  if (loaderError || !event) {
    return <NotFound title="Admin Access Denied" message={loaderError || "This event doesn't exist or the admin link is invalid."} />
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete response. Please try again.'
    ...
    <div className="min-h-screen bg-background py-12 px-4">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-foreground">{event.maxRespondents}</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-card backdrop-blur-sm border border-border rounded-xl p-6 mb-6">
          <h2 className="text-2xl font-bold text-foreground mb-4">Share Links</h2>
